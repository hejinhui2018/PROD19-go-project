...
func (s *Service) Transition(id string, to domain.State) error {
	...
	for _, n := range s.router.Build(b) {
...
func (s *Service) RegisterIncident(id, severity, reporter, summary string) (domain.Blockade, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	b, ok := s.blocks[id]
	if !ok {
	...
	if !allowed(b.State, domain.StateIncident) {
		return b, fmt.Errorf("%w: %s to %s", domain.ErrConflict, b.State, domain.StateIncident)
	}
	now := s.clk.Now()
	incident, err := safety.Escalate(safety.Incident{
		ID:         fmt.Sprintf("incident-%s-%d", id, b.Version+1),
		BlockadeID: id,
		Severity:   severity,
		Reporter:   reporter,
		Summary:    summary,
	...
	plan := safety.BuildEscalationPlan(incident)
	actions := make([]domain.IncidentAction, 0, len(plan.Immediate))
	for _, step := range plan.Immediate {
		actions = append(actions, domain.IncidentAction{Team: step.Team, DueAt: step.Deadline, Complete: step.Completed})
	}
	b.State = domain.StateIncident
	b.Version++
	b.UpdatedAt = now
	b.Incident = &domain.IncidentRecord{ID: incident.ID, Severity: incident.Severity, Reporter: incident.Reporter, Summary: incident.Summary, OpenedAt: incident.OpenedAt, Actions: actions}
	if err = s.persist("incident", b); err != nil {
		return b, err
	}
	_ = s.audit.Append(audit.New(fmt.Sprintf("%s-%d", id, b.Version), reporter, "incident", id, string(domain.StateActive), string(b.State), summary, now))
	for _, n := range s.router.Build(b) {
		if err = s.outbox.Add(n); err != nil {
			return b, err
		}
