...
func (s *Service) ReviewPlan(id string, revision int64) (domain.Blockade, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	b, ok := s.blocks[id]
	if !ok {
	...
	if revision <= b.PlanRevision {
		return b, fmt.Errorf("plan revision must advance")
	}
	b.PlanRevision = revision
	b.Version++
	b.UpdatedAt = s.clk.Now()
	if e := s.persist("plan_review", b); e != nil {
