...
func (w Workflow) ReviewPlan(id string, revision int64) (domain.Blockade, error) {
	return w.Service.ReviewPlan(id, revision)
}
...
func (w Workflow) Activate(id string) (domain.Blockade, error) {
	...
	if e = planning.TransitionSafe(d, true); e != nil {
		return b, e
	}
...
func (w Workflow) Incident(id string, summary string) (domain.Blockade, error) {
	...
	if summary == "" {
		return b, fmt.Errorf("incident summary required")
	}
	return b, w.Service.Transition(id, domain.StateIncident)
}
